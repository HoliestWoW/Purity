...
function MageModule:IsTalentForbidden(tabIndex)
    ...
    -- Corrected talent tab order: 1=Arcane, 2=Fire, 3=Frost.
...
function MageModule:EventHandler(event, ...)
    local currentDB = Purity:GetDB()
    local chosenSpec = currentDB.mageData and currentDB.mageData.specialization
    ...
    if event == "COMBAT_LOG_EVENT_UNFILTERED" then
        ...
        if sourceGUID == UnitGUID("player") and (subEvent == "SPELL_CAST_SUCCESS" or subEvent == "SPELL_AURA_APPLIED") then
            if self:IsSpellForbidden(spellId) then Purity:Violation("Cast a forbidden spell:\n" .. spellName) end
    ...
    elseif event == "SPELLS_CHANGED" then
        for i = 1, GetNumSpellTabs() do
            ...
            for j = 1, numSpells do
                local currentSpellID = GetSpellBookItemInfo(j, "spell")
                if currentSpellID and self:IsSpellForbidden(currentSpellID) then
                    Purity:Violation("Learned forbidden spell:\n" .. GetSpellInfo(currentSpellID))
                    return
                end
            end
        end
    elseif event == "PLAYER_TALENT_UPDATE" then
        local allowedTabIndex
        if chosenSpec == "Arcane" then allowedTabIndex = 1
        elseif chosenSpec == "Fire" then allowedTabIndex = 2
        elseif chosenSpec == "Frost" then allowedTabIndex = 3
        end
        if not allowedTabIndex then return end

        for i = 1, GetNumTalentTabs() do
            if i ~= allowedTabIndex then
                for j = 1, GetNumTalents(i) do
                    local _, _, _, _, pointsSpent = GetTalentInfo(i, j)
                    if pointsSpent > 0 then
                        local tabNames = { [1] = "Arcane", [2] = "Fire", [3] = "Frost" }
                        Purity:Violation("Allocated points in the forbidden\n".. (tabNames[i] or "Unknown") .." talent tree.")
                        return
                    end
                end
            end
