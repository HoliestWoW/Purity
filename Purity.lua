...
Purity.OriginalBuyMerchantItem = BuyMerchantItem
BuyMerchantItem = function(index, quantity)
    local purchaseQuantity = quantity or 1
    local db = Purity:GetDB()
    local itemName, _, itemPrice = GetMerchantItemInfo(index)

    Purity.OriginalBuyMerchantItem(index, quantity)

    if db.isOptedIn and db.challengeTitle == "The Drunken Master" and itemName and itemPrice and itemPrice > 0 and Purity.DRINK_LIST[itemName] then
        if not db.challengeStats then db.challengeStats = {} end

        db.challengeStats.moneySpent = (db.challengeStats.moneySpent or 0) + (itemPrice * purchaseQuantity)
        Purity:MarkDBDirty()

        -- This line now correctly checks the panel we exposed in the other file.
